...
tcr_genes = ['ERCC8', 'ERCC6', 'POLR2A', 'POLR2B', 'POLR2C', 'POLR2D', 'POLR2E', 'POLR2F', 'POLR2G', 'POLR2H', 
             'POLR2I', 'POLR2J', 'USP7', 'XAB2', 'UVSSA', 'POLR2M', 'POLR2J2', 'POLR2J3']

study_ids = ["laml_tcga_pan_can_atlas_2018", "acc_tcga_pan_can_atlas_2018",
             "meso_tcga_pan_can_atlas_2018", "ucs_tcga_pan_can_atlas_2018"]
study_names = ["Acute Myeloid Leukemia", "Adrenocortical Carcinoma",
               "Mesothelioma", "Uterine Carcinosarcoma"] 
...
for study_id in study_ids:
  p_values = []  # store p values of the genes in the same order as tcr_genes
  z_scores = []  # store z scores of the genes in the same order as tcr_genes
  for tcr_gene in tcr_genes:
    df = pd.read_csv("./tcr/" + study_id + "/cancer_type_summary_sample_percentage_tcr_" + tcr_gene + "_" + study_id + ".txt", sep="\t")
    df.set_index("Unnamed: 0", inplace=True)
    # calculate sample percentages of the singlegene's random lists
    cancer_type_summary_tcr_singlegene_sample_percentage = {}
    ...
    for randomlist, row in df.iterrows():
      ...
      cancer_type_summary_tcr_singlegene_sample_percentage[randomlist] = gene_dict

    df = pd.read_csv("./tcr/" + study_id + "/ner_sample_percentages_tcr_" + tcr_gene + "_" + study_id + ".txt", sep="\t")
    df.set_index("Unnamed: 0", inplace=True)
    ...
    for random_list, percentage_dict in cancer_type_summary_tcr_singlegene_sample_percentage.items():
...
# save p_values_all
with open("./tcr/p_values_tcr.txt", "w") as f:
  f.write("TCR_genes\t")
...
# save z_scores_all
with open("./tcr/z_scores_tcr.txt", "w") as f:
  f.write("TCR_genes\t")
  for gene in tcr_genes:
    f.write("\t" + gene)
  f.write("\n")
...
for i, gene in enumerate(tcr_genes):
...
ax.set_title('Significant Genes in Different Cancer Studies - TCR', fontsize=25)
...
plt.savefig('tcr/significant_genes_per_study_tcr.pdf', format='pdf')
