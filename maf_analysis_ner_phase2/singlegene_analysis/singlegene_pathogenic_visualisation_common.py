...
common_genes = ["CCNH", "CDK7", "DDB1", "ERCC1", "ERCC2", "ERCC3", "ERCC4", "ERCC5", 
                "GTF2H1", "GTF2H2", "GTF2H3", "GTF2H4", "LIG1", "MNAT1", "POLD1",
                "POLE", "RPA1", "RPA2", "RPA3", "XPA", "CUL4A", "RBX1", "POLD3"]

study_ids = ["kich_tcga_pan_can_atlas_2018"]
study_names = ["Kidney Chromophobe"] 
...
for study_id in study_ids:
  p_values = []  # store p values of the genes in the same order as common_genes
  z_scores = []  # store z scores of the genes in the same order as common_genes
  for common_gene in common_genes:
    df = pd.read_csv("./common/" + study_id + "/cancer_type_summary_sample_percentage_common_" + common_gene + "_" + study_id + ".txt", sep="\t")
    df.set_index("Unnamed: 0", inplace=True)
    # calculate sample percentages of the singlegene's random lists
    cancer_type_summary_common_singlegene_sample_percentage = {}
    ...
    for randomlist, row in df.iterrows():
      ...
      cancer_type_summary_common_singlegene_sample_percentage[randomlist] = gene_dict

    df = pd.read_csv("./common/" + study_id + "/ner_sample_percentages_common_" + common_gene + "_" + study_id + ".txt", sep="\t")
    df.set_index("Unnamed: 0", inplace=True)
    ...
    for random_list, percentage_dict in cancer_type_summary_common_singlegene_sample_percentage.items():
...
# save p_values_all
with open("./common/p_values_common.txt", "w") as f:
...
# save z_scores_all
with open("./common/z_scores_common.txt", "w") as f:
  f.write("common_genes\t")
  for gene in common_genes:
    f.write("\t" + gene)
  f.write("\n")
...
ax.set_title('Significant Genes in Different Cancer Studies - COMMON', fontsize=25)
...
plt.savefig('common/significant_genes_per_study_common.pdf', format='pdf')
