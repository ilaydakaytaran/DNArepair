...
def main(study_id):
  ...
  # get df of common
  df_common = df_all_repair[(df_all_repair['Pathway'].str.contains('NER')) & (df_all_repair['other_designations']=="GG, TCR")]
  ...
  df_common = df_common[df_common["protein_length"]>100]


  # get all potential background genes
  all_background_genes = {}  # {gene: [backgene1, backgene2, ...] for gene in df_common["Symbol"]} for study
  for gene in df_common["GeneID"].values:        # iterate genes in each study
    gene = int(gene)
    background_genes = []
    ner_length = int(df_common[df_common["GeneID"] == gene]["protein_length"].iloc[0])
  ...
  df.to_csv("./cancer_studies_background/" + study_id + "/all_background_genes_common_" + study_id + ".txt", sep="\t")
  ...
  for i in range(1000):    # prepare 1000 random lists
    ...
    for ner_gene, potential_back_genes_list in all_background_genes.items():  # iterate over the potential genes
      if (i == 0):
        ner_list.append(df_common[df_common['GeneID']==ner_gene]["Symbol"].values[0])
  ...
  df.to_csv("./cancer_studies_background/" + study_id + "/random_backgrounds_common_1000_" + study_id + ".txt", sep="\t") 

  # obtain mutated samples for background genes
  cancer_type_summary_common = {} # {randomlist: genedict} where genedict is dict of mutated sample ids for each gene in randomlist
  for randomlist, genes in random_backgrounds.items():
    ...
    if randomlist != "ner_list":
      cancer_type_summary_common[randomlist] = gene_dict

  # save cancer_type_summary_common
  df = pd.DataFrame(cancer_type_summary_common.values(), index = cancer_type_summary_common.keys())
  df.to_csv("./cancer_studies_background/" + study_id + "/cancer_type_summary_common_1000_" + study_id + ".txt", sep="\t")

  # obtain mutated samples for ner genes
  ner_gene_dict = {gene: [] for gene in df_common["Symbol"]}
  ...
  df.to_csv("./cancer_studies_background/" + study_id + "/ner_gene_dict_common_" + study_id + ".txt", sep="\t")

  # calculate mutation percentages for background genes
  cancer_type_summary_common_sample_percentage = {}
  for randomlist, gene_dict in cancer_type_summary_common.items():
    dct = {}
    ...
    cancer_type_summary_common_sample_percentage[randomlist] = dct

  # save cancer_type_summary_common_sample_percentage
  df = pd.DataFrame(cancer_type_summary_common_sample_percentage.values(), index = cancer_type_summary_common_sample_percentage.keys())
  df.to_csv("./cancer_studies_background/" + study_id + "/cancer_type_summary_sample_percentage_common_1000_" + study_id + ".txt", sep="\t")
  ...
  df.to_csv("./cancer_studies_background/" + study_id + "/ner_sample_percentages_common_" + study_id + ".txt", sep="\t")
