...
def main(study_id):
  ...
  df = pd.read_csv("./cancer_studies_background/" + study_id + "/cancer_type_summary_sample_percentage_common_1000_" + study_id + ".txt", sep="\t")
  df.set_index("Unnamed: 0", inplace=True)

  # calculate sample percentages
  cancer_type_summary_common_1000_sample_percentage = {}
  ...
  for randomlist, row in df.iterrows():
    ...
    cancer_type_summary_common_1000_sample_percentage[randomlist] = gene_dict

  df = pd.read_csv("./cancer_studies_background/" + study_id + "/ner_sample_percentages_common_" + study_id + ".txt", sep="\t")
  df.set_index("Unnamed: 0", inplace=True)
  ...
  for random_list, percentage_dict in cancer_type_summary_common_1000_sample_percentage.items():
  ...
  plt.title(study_id + ' Mutations - COMMON - 1000 Lists', fontweight='bold', fontsize = 14)
  ...
  plt.savefig("./cancer_studies_background/" + study_id + '/mut_cancer_type_summary_common_1000_' + study_id + '.pdf', format='pdf')
  ...
  with open("./cancer_studies_background/" + study_id + "/p_value_common.txt", "w") as f:
  ...
  plt.savefig("./cancer_studies_background/" + study_id + '/mut_norm_dist_common_1000_' + study_id + '.pdf', format='pdf')
