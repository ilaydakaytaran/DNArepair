...
def main(study_id):
  ...
  all_background_genes = {}  # {gene: [backgene1, backgene2, ...] for gene in df_all["Symbol"]} for study
  for gene in df_all["GeneID"].values:        # iterate genes in each study
    gene = int(gene)
    background_genes = []
    ner_length = int(df_all[df_all["GeneID"] == gene]["protein_length"].iloc[0])
  ...
  df.to_csv("./cancer_studies/" + study_id + "/all_background_genes_all_" + study_id + ".txt", sep="\t")
  ...
  for i in range(1000):    # prepare 1000 random lists
    ...
    for ner_gene, potential_back_genes_list in all_background_genes.items():  # iterate over the potential genes
      if (i == 0):
        ner_list.append(df_all[df_all['GeneID']==ner_gene]["Symbol"].values[0])
  ...
  df.to_csv("./cancer_studies/" + study_id + "/random_backgrounds_all_1000_" + study_id + ".txt", sep="\t") 
  ...
  df.to_csv("./cancer_studies/" + study_id + "/cancer_type_summary_all_1000_" + study_id + ".txt", sep="\t")

  # obtain mutated samples for ner genes
  ner_gene_dict = {gene: [] for gene in df_all["Symbol"]}
  ...
  df.to_csv("./cancer_studies/" + study_id + "/ner_gene_dict_all_" + study_id + ".txt", sep="\t")
  ...
  df.to_csv("./cancer_studies/" + study_id + "/cancer_type_summary_sample_percentage_all_1000_" + study_id + ".txt", sep="\t")
  ...
  df.to_csv("./cancer_studies/" + study_id + "/ner_sample_percentages_all_" + study_id + ".txt", sep="\t")
