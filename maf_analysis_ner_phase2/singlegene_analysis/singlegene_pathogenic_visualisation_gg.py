...
GG_genes = ["CETN2", "DDB2", "RAD23A", "RAD23B", "RNF111", "XPC"]

study_ids = ["blca_tcga_pan_can_atlas_2018", "chol_tcga_pan_can_atlas_2018", "esca_tcga_pan_can_atlas_2018",
            "lihc_tcga_pan_can_atlas_2018", "ov_tcga_pan_can_atlas_2018"]
study_names = ["Bladder Urothelial Carcinoma", "Cholangiocarcinoma", "Esophageal Adenocarcinoma",
...
for study_id in study_ids:
  p_values = []  # store p values of the genes in the same order as GG_genes
  z_scores = []  # store z scores of the genes in the same order as GG_genes
  for gg_gene in GG_genes:
    df = pd.read_csv("./gg/" + study_id + "/cancer_type_summary_sample_percentage_gg_" + gg_gene + "_" + study_id + ".txt", sep="\t")
    df.set_index("Unnamed: 0", inplace=True)
    # calculate sample percentages of the singlegene's random lists
    cancer_type_summary_gg_singlegene_sample_percentage = {}
    ...
    for randomlist, row in df.iterrows():
      ...
      cancer_type_summary_gg_singlegene_sample_percentage[randomlist] = gene_dict

    df = pd.read_csv("./gg/" + study_id + "/ner_sample_percentages_gg_" + gg_gene + "_" + study_id + ".txt", sep="\t")
    df.set_index("Unnamed: 0", inplace=True)
    ...
    for random_list, percentage_dict in cancer_type_summary_gg_singlegene_sample_percentage.items():
...
# save p_values_all
with open("./gg/p_values_gg.txt", "w") as f:
  f.write("GG_genes\t")
  for gene in GG_genes:
    f.write("\t" + gene)
  f.write("\n")
...
# save z_scores_all
with open("./gg/z_scores_gg.txt", "w") as f:
  f.write("GG_genes\t")
  for gene in GG_genes:
    f.write("\t" + gene)
  f.write("\n")
...
for i in range(len(study_ids) - 1): 
  ax.vlines(i + 0.5, ymin=0, ymax=8, linestyle='dotted', color='grey', alpha=0.5)
...
ax.set_title('Significant Genes in Different Cancer Studies - GG', fontsize=25)
...
plt.savefig('gg/significant_genes_per_study_gg.pdf', format='pdf')
