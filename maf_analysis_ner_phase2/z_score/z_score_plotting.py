...
if len(list(z_scores_all.keys())) > 0:
    ...
    plt.ylabel('Z-score')
    plt.title('Mutations - all - Significant Z-Scores', fontweight='bold', fontsize = 14)
    plt.xticks(x_indices, study_names_all, rotation=0, fontsize=9, ha='center')
...
if len(list(z_scores_all.keys())) > 0:
    ...
    plt.xlabel('Study Name')
    plt.ylabel('Z-score')
    plt.title('Pathogenic Mutations - ALL - Significant Z-Scores', fontweight='bold', fontsize = 14)
    plt.xticks(x_indices, study_names_all, rotation=0, fontsize=9, ha='center')
    plt.yticks(fontsize=10)
    plt.grid(axis='y', linestyle='--', alpha=0.2, color="#2a9d8f")
    plt.gca().spines['top'].set_visible(False)
